...
type ContactApiResponse = { ok: true } | { ok: false; errorCode?: string; message?: string };
...
export function ContactForm({ onSentAction }: ContactFormProps) {
  ...
  async function onSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    if (res.ok) {
      ...
      const code =
        responseBody && 'ok' in responseBody && !responseBody.ok
          ? responseBody.errorCode
          : undefined;

      if (code === 'contact_rejected') {
        setErrorMessage(
          'That message can’t be sent as written. Try rephrasing (no links, commands, or aggressive language).',
        );
